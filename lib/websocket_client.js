...
module.exports = class WebsocketClient extends EventEmitter
{
	constructor( socket, options = {})
	{
		...
		this._options = Options( options,
		{
			...
			headers	: { _required: false, _type: 'object' },
			frame	:
			{
				_expand	: true,
		...
		this._last_data_received = 0;
		this._last_data_sent = 0;
	...
	_initialize( socket )
	{
		...
		this._socket = socket;
		this._last_data_sent = this._last_data_received = Date.now();
		...
		this.on( 'pong', payload => { this._last_data_received = Date.now() });
		this.on( 'closing', ( code, reason ) => this.close( code, reason ));

		socket.on( 'data', data =>
		{
			this._last_data_received = Date.now();
	...
	get last_data_received(){ return this._last_data_received; }
	get last_data_sent(){ return this._last_data_sent; }
}
